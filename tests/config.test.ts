...
test("loadConfig allows localhost HTTP transport without auth token", () => {
  const config = loadConfig({
    MCP_TRANSPORT: "http-sse",
    MCP_HTTP_BIND_HOST: "127.0.0.1"
  });

  assert.equal(config.transport, "http-sse");
  assert.equal(config.httpAuthToken, undefined);
});

test("loadConfig requires auth token when HTTP transport binds externally", () => {
  assert.throws(
    () =>
      loadConfig({
        MCP_TRANSPORT: "http-sse",
    ...
    /MCP_HTTP_AUTH_TOKEN is required/i
  );
});

test("loadConfig accepts auth token when HTTP transport binds externally", () => {
  const config = loadConfig({
    MCP_TRANSPORT: "http-sse",
    MCP_HTTP_BIND_HOST: "0.0.0.0",
    MCP_HTTP_AUTH_TOKEN: "dev-token"
  });

  assert.equal(config.httpAuthToken, "dev-token");
});
