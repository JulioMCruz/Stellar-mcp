...
export function validateMcpAuthRequest(
  req: IncomingMessage,
  ...
  if (typeof authorization !== "string") {
    return {
      ok: false,
      status: 401,
      error: "POST /mcp requires Authorization: Bearer <token>."
    };
  }
  ...
  if (!valid) {
    return {
      ok: false,
      status: 403,
      error: "Invalid MCP HTTP authorization token."
    };
  }
...
export async function startHttpServer(config: AppConfig): Promise<Server> {
  ...
  const server = createHttpServer(async (req, res) => {
    ...
    if (url.pathname === "/mcp") {
      ...
      if (req.method === "POST") {
        const authValidation = validateMcpAuthRequest(req, config.httpAuthToken);
        if (!authValidation.ok) {
          res.writeHead(authValidation.status, { "content-type": "application/json" });
